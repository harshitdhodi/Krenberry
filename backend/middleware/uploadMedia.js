        ...
        if (req.files && req.files['photo']) {
            ...
            const processPromises = photos.map(async (photo) => {
                ...
                try {
                    ...
                    // Check if the file is a WebM (video) file or GIF
                    const isWebM = photo.mimetype === 'video/webm' || path.extname(photo.filename).toLowerCase() === '.webm';
                    const isGIF = photo.mimetype === 'image/gif' || path.extname(photo.filename).toLowerCase() === '.gif';
                    
                    if (isWebM || isGIF) {
                        // For WebM and GIF files, just move them to the final location
